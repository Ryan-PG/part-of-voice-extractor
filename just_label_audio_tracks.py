...
def extract_number(filename):
    ...
    return int(match.group()) if match else float('inf')  # Put non-numbered files at the end

def build_csv_from_labels(labels_dir, voices_dir, output_dir):
    file_type = input("Enter the file type ('mp3' or 'wav' or 'm4a'): (default = wav) ").strip().lower()
    ...
    with open(label_path, 'r', encoding='utf-8') as lf, open(csv_path, mode='w', newline='', encoding='utf-8') as csv_file:
        ...
        csv_writer.writerow(["Label", "File Name"])
        ...
        for idx, label in enumerate(lines):
            ...
            csv_writer.writerow([label, audio_file])
            print(f"Mapped label '{label}' to audio file '{audio_file}'")
